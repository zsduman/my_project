...
try:
    while True:
        ...
        # Veriyi örnek veri listesine ekle
        sample_data.append((cTemp, fTemp, humidity))
        
        # Veriyi filtrele ve medyanını al
        cTemp_median = moving_median([data[0] for data in sample_data])
        fTemp_median = moving_median([data[1] for data in sample_data])
        humidity_median = moving_median([data[2] for data in sample_data])
